  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:3001/api/resume-builder/",
        {
          method: "POST",
          headers: {
    ...
    <div className="container mx-auto px-4 py-10">
      <div className="flex justify-between items-center mb-8">
        <div className="flex-1">
          <span className="text-2xl font-extrabold text-left text-purple-600">
            InterWin
          </span>
        </div>
        <div className="flex-1">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {formData.skills.map((skill, index) => (
            <div key={index} className="mb-2 flex">
              <input
                ...
                value={skill}
                onChange={(e) =>
                  handleArrayInputChange(index, "skills", null, e.target.value)
                }
          ...
          {formData.hobbies.map((hobby, index) => (
            <div key={index} className="mb-2 flex">
              <input
                ...
                value={hobby}
                onChange={(e) =>
                  handleArrayInputChange(index, "hobbies", null, e.target.value)
                }
                className="flex-grow px-3 py-2 border border-slate-300 rounded mr-2"
                required
              />
        ...
        <div className="bg-purple-50 p-6 rounded-lg shadow">
          ...
          {formData.certifications.map((cert, index) => (
            <div
              key={index}
              className="mb-4 p-4 border border-slate-300 rounded"
            >
              <input
                type="text"
